...
import fit.iuh.order.module.order_management.controller.PaymentController;
import fit.iuh.order.module.config.RabbitMQConfig;
    ...
    public void consumePaymentSettledEvent(PaymentSettledEvent event) {
        ...
        try {
            ...
            if (order != null) {
                ...
                // 1. Cập nhật trạng thái đơn hàng sang CONFIRMED qua State Pattern
                if (order.getStatus() == OrderStatus.PENDING) {
                    order.changeState(new PendingState());
                    orderRepository.save(order);
                    System.out.println("RabbitMQ Consumer: Đã chuyển đơn hàng #" + orderId + " sang trạng thái CONFIRMED.");
                }
